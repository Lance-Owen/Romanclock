...
def design(t,num,name,cent_x,cent_y,R):
    y1 = [str(i) + name for i in range(t.month, num)]
    y2 = [str(i) + name for i in range(1, t.month)]
    y1.extend(y2)
    for i in range(0, num):
        text = font.render(str(y1[i]), True, (255, 255, 255), 1)
        new_text = pygame.transform.rotozoom(text, 360 - (360/num) * i, 1)
        screen.blit(new_text,
                    (cent_x + R * math.cos((2 * math.pi / num) * i), cent_y + R * math.sin((2 * math.pi / num) * i)))
